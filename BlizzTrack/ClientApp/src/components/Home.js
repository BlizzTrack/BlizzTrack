...
export class Home extends Component {
  ...
  update(seqn) {
    BlizzTrack.Call(`NGPD/summary?seqn=${seqn ?? ""}`).then(data => {
      ...
      <div className="col-12">
        {!this.state.summary ? "Loading..." :
          <div>
            ...
            <ListGroup>
              {this.state.summary.data.map((item) => {
                const clicked = () => {
                  var a = this.state.actions ?? {};
                ...
                return <div className="row p-1 border game-item" key={`${item.product}_${item.flags}`}>
